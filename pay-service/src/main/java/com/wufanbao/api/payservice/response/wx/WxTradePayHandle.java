...
public class WxTradePayHandle implements IResponseHandle {
    private ResWxTradePay.Data data;

    public void setData(ResWxTradePay.Data data) {
        this.data = data;
    }

    @Override
    public void decodeData(ResponseBody responseBody) throws Exception {
        ResponseData responseData = new Gson().fromJson(responseBody.getData(), new TypeToken<ResponseData<ResWxTradePay>>() {
        }.getType());
        if (responseData.getCode() != "100") {
            throw new Exception(responseData.getMessage());
        }
        ResWxTradePay resWxWapPay = (ResWxTradePay) responseData.getBiz_content();


        ResWxTradePay.Data data = new ResWxTradePay.Data();
    ...
    @Override
    public ResWxTradePay.Data getData() {
        return this.data;
    }
